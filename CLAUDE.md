...
- **Seed data**: `Data/DbSeeder.cs` now seeds only the three operational roles and one initial Admin account (`admin@trailguard.com`), sourced from `IConfiguration["SeedAdmin:Password"]` (Local) or `IConfiguration["SeedAdmin:SupabasePassword"]` (Supabase) depending on the resolved `Database:Target` — see `Services/DatabaseTargetResolver.cs` — with no fallback between them. There is no longer any Organizer/Participant sample account, Trail, Event, or registration seeding, commented out or otherwise; a development catalog (Trails, Events, sample Organizer/Participant accounts) is created manually through the app once signed in as Admin.
