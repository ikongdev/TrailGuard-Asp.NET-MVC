...
`Data/DbSeeder.cs` (`SeedRolesAndAdminAsync`, called once from `Program.cs` on every startup) seeds exactly two things, and nothing else:

1. The three operational roles (`Admin`, `Organizer`, `Participant`), read from `OperationalRolePolicy.AllowedRoles` — the same allow-list every other role-creation/role-edit path uses (see "Account Roles").
2. One initial Admin account, `admin@trailguard.com`, created only if no account with that email already exists.
...
- `Data/DbSeeder.cs` no longer seeds any Trail (the former twelve agency Trails were removed from the seeder along with the Organizer/Participant sample accounts — see "Startup Seeding," below); every Trail and Event now originates from Trail Management / Add Event.
...
| `Data/DbSeeder.cs` | No longer seeds any Trail at all (see "Startup Seeding," below) — not applicable to `IsActive` filtering |
...
`Services/RoleAssignmentService.cs` (paired with the static `OperationalRolePolicy`) is the single source of truth for the exact allow-listed role names, reading an account's role integrity (`Admin` / `Organizer` / `Participant` / `Conflict` / `Missing`), and creating a brand-new account with its initial role. `AccountController.Register`, `AdminController.AddAccount`, and `Data/DbSeeder.cs` use that initial-assignment flow rather than calling `UserManager.CreateAsync`/`AddToRoleAsync` directly.
...
**Existing multi-role or role-less accounts are never auto-mutated.** Startup only *audits* and logs an aggregate warning (`Program.cs`, via `RoleAssignmentService.AuditRoleIntegrityAsync`), and Account Management displays the associated warning without a repair action. There is no "keep the highest role" or "navbar precedence" normalization anywhere in the codebase.
...
`RoleAssignmentService.SetAccountActiveAsync` (used by `AdminController.ToggleAccountStatus`) preserves the last-Admin protection for account deactivation.
...
**Every active-status change updates the target's security stamp**, but this does not terminate an already-issued cookie immediately — ASP.NET Core Identity's default `SecurityStampValidator` interval (30 minutes; `Program.cs` never overrides it) is the actual bound on how long a stale session keeps its old claims.

**Last-Admin deactivation checks are concurrency-protected**, not just ordered-before-the-write: disabling an account through `SetAccountActiveAsync` re-reads the target and re-counts other Admins inside a `Serializable` transaction (Npgsql/PostgreSQL), so two concurrent requests can't both disable the last active Admin — PostgreSQL aborts the second with a `40001 serialization_failure`, which surfaces as a generic "changed concurrently, try again" result. Re-enabling an account keeps the provider's default isolation.
...
- **Seed data**: `Data/DbSeeder.cs` now seeds only the three operational roles and one initial Admin account (`admin@trailguard.com`) — see "Startup Seeding," below. There is no longer any Organizer/Participant sample account, Trail, Event, or registration seeding, commented out or otherwise; a development catalog (Trails, Events, sample Organizer/Participant accounts) is created manually through the app once signed in as Admin
